...
public class LRCorrelator extends JFrame {
    ...
    private void processSelectedValue() {
        ...
        int selStart = actionPane.getSelectionStart();
        ...
        // that comes BEFORE the request that uses it. If the earliest response
        // containing it is the SAME snapshot (or later) as the request where it
        // is used, there is no valid source - the request is already sent by the
        // time that response arrives - so treat it as "not found".
        String usageSnap = findUsageSnapshot(rawActionText, selStart);
        int srcN = snapshotNumber(result.snapshotId);
        int useN = snapshotNumber(usageSnap);
        if (srcN >= 0 && useN >= 0 && srcN >= useN) {
            JOptionPane.showMessageDialog(this,
                "\"" + truncate(dyn, 40) + "\" is only found in the response of Snapshot=" +
                result.snapshotId + ",\nwhich is the same as or later than where it is used " +
                "(Snapshot=" + usageSnap + ").\n\n" +
    ...
    // The snapshot of the request block in the ACTION file that contains `pos`
    // (i.e. where the selected value is USED). Walks back to the enclosing web_*
    // call, then reads that block's Snapshot=tN.inf.
    private String findUsageSnapshot(String action, int pos) {
        if (action == null || action.isEmpty()) return "";
        if (pos < 0) pos = 0;
        if (pos > action.length()) pos = action.length();
        int blockStart = 0;
        Matcher m = Pattern.compile(
            "(?im)^\\s*web_(?:url|submit_data|custom_request|submit_form|link)\\s*\\(").matcher(action);
        while (m.find()) {
            if (m.start() <= pos) blockStart = m.start();
            else break;
        }
        Matcher sm = Pattern.compile("Snapshot=([A-Za-z0-9_]+)\\.inf",
            Pattern.CASE_INSENSITIVE).matcher(action);
        if (sm.find(blockStart)) return sm.group(1) + ".inf";
        return "";
    }
