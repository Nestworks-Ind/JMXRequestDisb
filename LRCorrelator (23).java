...
public class LRCorrelator extends JFrame {
    ...
    private static class ReqBlock {
        int start, end; String text, host, path, name, method; boolean disabled;
    }
    ...
    private List<ReqBlock> parseRequestBlocks(String action) {
        ...
        while (fn.find(from)) {
            ...
            Matcher lm = lastP.matcher(action);
            int end = lm.find(fn.end()) ? lm.end() : action.length();
            String block = action.substring(start, end);
            ReqBlock b = new ReqBlock();
            b.start = start; b.end = end; b.text = block; b.disabled = disabled;
    ...
    private void requestsDialog() {
        ...
        JCheckBox cbUrl = darkCheck("URL/Host", true), cbPath = darkCheck("Path", true),
                  cbName = darkCheck("Name", true), cbRegex = darkCheck("Regex", false), cbCase = darkCheck("Case", false);
        ...
        refreshHosts.run(); runSearch.run();
        ...
        sBar.add(darkLabel("Find:")); tfSearch.setColumns(20); sBar.add(tfSearch);
        sBar.add(cbUrl); sBar.add(cbPath); sBar.add(cbName); sBar.add(cbRegex); sBar.add(cbCase); sBar.add(searchBtn);
        ...
        java.util.function.IntConsumer act = mode -> {   // 0 disable, 1 enable, 2 delete
            List<ReqBlock> targets = collectTargets(tabs.getSelectedIndex(), hostTable, hostModel, searchTable, searchModel, blocksRef[0]);
            if (targets.isEmpty()) { JOptionPane.showMessageDialog(d, "Select host(s) or request(s) first (tick the checkboxes).", "Requests", JOptionPane.INFORMATION_MESSAGE); return; }
            if (mode == 2 && JOptionPane.showConfirmDialog(d, "Delete " + targets.size() + " request block(s)? This cannot be undone here.", "Delete", JOptionPane.OK_CANCEL_OPTION) != JOptionPane.OK_OPTION) return;
            ...
            refreshHosts.run(); runSearch.run();
            setStatus("[REQ] " + (mode==0?"Disabled ":mode==1?"Enabled ":"Deleted ") + targets.size() + " request(s).", STATUS_OK);
        };
    ...
    private List<ReqBlock> collectTargets(int tab, JTable hostTable, DefaultTableModel hostModel,
        ...
        if (tab == 0) {
            ...
            // match visible rows back to blocks by (name, host, path)
