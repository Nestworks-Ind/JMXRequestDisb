...
public class ScriptCompare extends JFrame {
    ...
    static final class Row {
        ...
        RowType type;
        List<int[]> leftHi, rightHi;  // multiple changed spans {startCol,endCol}
        String[] leftSeg, rightSeg;   // wrapped visual lines for each side
    }
    ...
    private JComponent buildDiffCard() {
        ...
        ComponentAdapter onResize = new ComponentAdapter() {
            public void componentResized(ComponentEvent e) { if (wrap && diffShowing) relayout(); }
        };
        leftDiffScroll.getViewport().addComponentListener(onResize);
        rightDiffScroll.getViewport().addComponentListener(onResize);
    ...
    private JToolBar buildToolBar() {
        ...
        JCheckBox cbWrap = new JCheckBox("Word wrap");
        cbWrap.setFocusable(false);
        cbWrap.addActionListener(e -> { wrap = cbWrap.isSelected(); if (diffShowing) relayout(); });
        tb.add(cbWrap);
        tb.addSeparator();
    ...
    private void compareFromText() {
        ...
        relayout();
        updateStats();
        SwingUtilities.invokeLater(() -> { relayout(); leftDiffScroll.getVerticalScrollBar().setValue(0); });
    }
    ...
    private void gotoBlock(int dir) {
        ...
        int rowY = (b[0] < leftPanel.rowTop.length) ? leftPanel.rowTop[b[0]] : b[0] * lh;
        int target = Math.max(0, rowY - lh * 2);
    ...
    private void changeFont(int delta) {
        ...
        if (diffShowing) relayout();
    }

    // ---- layout / wrapping geometry -----------------------------------
    private int viewportWidth(JScrollPane sp) {
        int w = sp.getViewport().getExtentSize().width;
        return w > 0 ? w : Math.max(300, sp.getWidth() - 4);
    }

    private void setScrollPolicies() {
        int pol = wrap ? ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER
                       : ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED;
        leftDiffScroll.setHorizontalScrollBarPolicy(pol);
        rightDiffScroll.setHorizontalScrollBarPolicy(pol);
    }

    /** Recompute per-row visual heights and segments for both panes. */
    private void relayout() {
        leftPanel.setWrap(wrap);
        rightPanel.setWrap(wrap);
        setScrollPolicies();

        int n = rows.size();
        if (n == 0) {
            leftPanel.setGeometry(new int[0], new int[0], 0);
            rightPanel.setGeometry(new int[0], new int[0], 0);
            return;
        }
        int lh = leftPanel.lineHeight;
        int cw = leftPanel.charWidth;
        int gw = leftPanel.gutterW;
        int[] rowTop = new int[n];
        int[] rowH = new int[n];

        if (!wrap) {
            for (int i = 0; i < n; i++) {
                Row r = rows.get(i);
                r.leftSeg = (r.left == null) ? EMPTY : new String[]{r.left};
                r.rightSeg = (r.right == null) ? EMPTY : new String[]{r.right};
                rowH[i] = lh;
            }
        } else {
            int lc = Math.max(1, (viewportWidth(leftDiffScroll) - gw - 2 * leftPanel.pad) / cw);
            int rc = Math.max(1, (viewportWidth(rightDiffScroll) - gw - 2 * rightPanel.pad) / cw);
            for (int i = 0; i < n; i++) {
                Row r = rows.get(i);
                r.leftSeg = wrapText(r.left, lc);
                r.rightSeg = wrapText(r.right, rc);
                int vl = Math.max(1, Math.max(r.leftSeg.length, r.rightSeg.length));
                rowH[i] = vl * lh;
            }
        }
        int y = 0;
        for (int i = 0; i < n; i++) { rowTop[i] = y; y += rowH[i]; }
        leftPanel.setGeometry(rowTop, rowH, y);
        rightPanel.setGeometry(rowTop, rowH, y);
    }

    /** Char-based wrap that prefers to break after a space. Preserves spacing. */
    private static String[] wrapText(String s, int width) {
        ...
        while (start < n) {
            int end = Math.min(start + width, n);
            if (end < n) {
                int brk = -1;
                for (int j = end; j > start; j--) {
                    if (s.charAt(j - 1) == ' ') { brk = j; break; }
                }
                if (brk > start + width / 3) end = brk; // break at space only if reasonable
            }
    ...
    /**
     * Token-level inline diff (Notepad++ style). Splits each side into word
     * tokens (maximal alphanumeric/underscore runs) and single-char separators,
     * runs the same Myers diff on the tokens, and emits a tight highlight span
     * for each changed region -- so multiple separate changes on one line are
     * highlighted individually instead of as one big block. Each changed gap is
     * then trimmed to its exact differing characters via common prefix/suffix.
     */
    private static void inlineDiff(Row r) {
        String L = r.left, R = r.right;
        int[][] lt = tokenize(L), rt = tokenize(R);
        String[] la = new String[lt.length];
        for (int i = 0; i < lt.length; i++) la[i] = L.substring(lt[i][0], lt[i][1]);
        String[] ra = new String[rt.length];
        for (int i = 0; i < rt.length; i++) ra[i] = R.substring(rt[i][0], rt[i][1]);

        List<Edit> edits = buildEdits(la, ra);
        List<int[]> lHi = new ArrayList<>(), rHi = new ArrayList<>();
        int dS = -1, dE = -1, iS = -1, iE = -1; // current gap: del-range on left, ins-range on right

        for (Edit e : edits) {
            if (e.t == EType.DEL) { if (dS < 0) dS = lt[e.ai][0]; dE = lt[e.ai][1]; }
            else if (e.t == EType.INS) { if (iS < 0) iS = rt[e.bi][0]; iE = rt[e.bi][1]; }
            else { flushGap(L, R, dS, dE, iS, iE, lHi, rHi); dS = dE = iS = iE = -1; }
        }
        flushGap(L, R, dS, dE, iS, iE, lHi, rHi);

        r.leftHi = lHi.isEmpty() ? null : lHi;
        r.rightHi = rHi.isEmpty() ? null : rHi;
    }
    ...
    /** Split into {start,end} tokens: alphanumeric runs, else single chars. */
    ...
    /** Trim a changed gap to its exact differing chars and record spans. */
    private static void flushGap(String L, String R, int dS, int dE, int iS, int iE,
                                 List<int[]> lHi, List<int[]> rHi) {
        if (dS < 0 && iS < 0) return;
        String lsub = (dS < 0) ? "" : L.substring(dS, dE);
        String rsub = (iS < 0) ? "" : R.substring(iS, iE);
        int minlen = Math.min(lsub.length(), rsub.length());
        int p = 0;
        while (p < minlen && lsub.charAt(p) == rsub.charAt(p)) p++;
        int suf = 0;
        while (suf < lsub.length() - p && suf < rsub.length() - p
                && lsub.charAt(lsub.length() - 1 - suf) == rsub.charAt(rsub.length() - 1 - suf)) suf++;
        if (dS >= 0) { int a = dS + p, b = dE - suf; if (a < b) lHi.add(new int[]{a, b}); }
        if (iS >= 0) { int a = iS + p, b = iE - suf; if (a < b) rHi.add(new int[]{a, b}); }
    }
    ...
    final class SidePanel extends JComponent implements Scrollable {
        ...
        // shared geometry set by relayout()
        int[] rowTop = new int[0];
        int[] rowH = new int[0];
        int totalHeight = 0;
        boolean wrap = false;
        ...
        void setGeometry(int[] rowTop, int[] rowH, int totalHeight) {
            this.rowTop = rowTop;
            this.rowH = rowH;
        ...
        void setWrap(boolean w) { this.wrap = w; }
        ...
        /** largest row index whose top <= y (binary search) */
        private int rowAt(int y) {
            int lo = 0, hi = rowTop.length - 1, ans = 0;
            while (lo <= hi) {
                int mid = (lo + hi) >>> 1;
                if (rowTop[mid] <= y) { ans = mid; lo = mid + 1; } else hi = mid - 1;
            }
            return ans;
        }

        @Override public Dimension getPreferredSize() {
            int h = Math.max(1, totalHeight);
            if (wrap) return new Dimension(gutterW + pad + 50, h);
        ...
        @Override protected void paintComponent(Graphics g0) {
            ...
            if (rows.isEmpty() || rowTop.length != rows.size()) return;
            ...
            for (int i = first; i < rows.size(); i++) {
                ...
                int h = rowH[i];
                Row r = rows.get(i);
                String[] segs = isLeft ? r.leftSeg : r.rightSeg;
                if (segs == null) segs = EMPTY;

                g.setColor(bgFor(r));
                g.fillRect(0, top, getWidth(), h);

                List<int[]> hi = (r.type == RowType.CHANGED) ? (isLeft ? r.leftHi : r.rightHi) : null;
                int off = 0;
                for (int s = 0; s < segs.length; s++) {
                    String seg = segs[s];
                    int y = top + s * lineHeight;
                    int segEnd = off + seg.length();
                    if (hi != null) {
                        g.setColor(isLeft ? IN_DELETED : IN_ADDED);
                        for (int[] span : hi) {
                            int a = Math.max(span[0], off);
                            int b = Math.min(span[1], segEnd);
                            if (b > a) {
                                int hx = gutterW + pad + (a - off) * charWidth;
                                g.fillRect(hx, y, (b - a) * charWidth, lineHeight);
                            }
                        }
                    }
                    g.setColor(Color.BLACK);
                    g.drawString(seg, gutterW + pad, y + ascent);
                    off = segEnd;
                }

                // pinned gutter spanning the whole (possibly multi-line) row
                g.setColor(GUTTER_BG);
                g.fillRect(gx, top, gutterW, h);
                g.setColor(GUTTER_LN);
                g.drawLine(gx + gutterW - 1, top, gx + gutterW - 1, top + h);
        ...
        public boolean getScrollableTracksViewportWidth() { return wrap; }
