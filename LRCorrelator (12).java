...
public class LRCorrelator extends JFrame {
    ...
    private String findUsageSnapshot(String action, String dyn) {
        ...
        while (fn.find()) starts.add(fn.start());
        for (int i = 0; i < starts.size(); i++) {
            int s = starts.get(i);
            int e = (i + 1 < starts.size()) ? starts.get(i + 1) : action.length();
            String block = action.substring(s, e);
            String flat = block.replaceAll("\"\\s*\"", "");   // join VuGen line-wrap seams
            if (flat.contains(key)) {
                Matcher sm = Pattern.compile("Snapshot=([A-Za-z0-9_]+)\\.inf",
                    Pattern.CASE_INSENSITIVE).matcher(block);
                if (sm.find()) return sm.group(1) + ".inf";
                return "";
            }
        }
        return "";
