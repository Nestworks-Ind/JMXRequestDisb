...
public class LRCorrelator extends JFrame {
    ...
    private JPanel buildMainLayout() {
        ...
        JPanel studioWrapper = buildCorrPanel();
        studioWrapper.setPreferredSize(new Dimension(0, 240));
        studioWrapper.setMinimumSize(new Dimension(0, 180));
        JSplitPane editors = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
                buildEditorPanel("ACTION FILE",          true,  BTN_BLUE),
                buildEditorPanel("CODE GENERATION FILE", false, BTN_GREEN));
        editors.setDividerLocation(0.5);
        editors.setResizeWeight(0.5);
        editors.setDividerSize(5);
        editors.setBackground(BG_DEEP);
        editors.setBorder(null);
        JSplitPane vertical = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
                studioWrapper, editors);
        vertical.setDividerLocation(240);
        vertical.setResizeWeight(0.0);
        vertical.setDividerSize(5);
        vertical.setBackground(BG_DEEP);
        vertical.setBorder(null);
        main.add(vertical, BorderLayout.CENTER);
        return main;
    }
    ...
    private JPanel buildEditorPanel(String title, boolean isAction, Color accent) {
        ...
        tp.setBackground(BG_EDITOR);
        tp.setForeground(FG_DEFAULT);
        tp.setFont(MONO_FONT);
        tp.setCaretColor(FG_DEFAULT);
        tp.setBorder(BorderFactory.createEmptyBorder(6, 8, 6, 8));
        ...
        scroll.setRowHeaderView(new LineNumberGutter(tp));

        // ---------------------------------------------------------------------
        // Keep the raw-text mirror in sync with WHATEVER lands in the pane -
        // typed, pasted, or loaded from a file. Previously rawActionText /
        // rawCodeGenText were only populated by loadFile(), so pasting content
        // in directly left them empty and Process Selection reported
        // "load the CodeGen file first". A DocumentFilter normalises line
        // endings on input so the offsets used later for boundary highlighting
        // stay stable, and a DocumentListener mirrors every edit back to the
        // raw string.
        // ---------------------------------------------------------------------
        ((AbstractDocument) tp.getDocument()).setDocumentFilter(new NewlineFilter());
        tp.getDocument().addDocumentListener(new DocumentListener() {
            private void sync() {
                String content = safeDocText(tp);
                if (isAction) rawActionText  = content;
                else          rawCodeGenText = content;
            }
            public void insertUpdate(DocumentEvent e)  { sync(); }
            public void removeUpdate(DocumentEvent e)  { sync(); }
            public void changedUpdate(DocumentEvent e) { /* attribute-only change, text unchanged */ }
        });

        SearchPanel searchBar = new SearchPanel(tp);
        searchBar.setVisible(false);
        if (isAction) {
            actionPane   = tp;
            actionSearch = searchBar;
            attachActionContextMenu(tp);
        } else {
            codeGenPane   = tp;
    ...
    private void processSelectedValue() {
        ...
        highlightBoundariesInCodeGen(result.lb, dyn, result.rb, result.sectionStart);
        showStudio(result);
    }
    ...
    private CorrelationResult runEngine(String dyn, String codeGen) {
        int dynIdx = codeGen.indexOf(dyn);
        if (dynIdx < 0) return null;

        // SNAPSHOT ID:
        // Search FORWARD from the dynamic value for "Snapshot=tX.inf"
        // This is exactly how you find it in Notepad++ - find the value,
        // then search forward for .inf, read Snapshot=tX
        String snapshotId = extractSnapshotId(codeGen, dynIdx);

        // SOURCE TYPE:
        // Search BACKWARD from the dynamic value.
        // Find nearest "Response Body for" OR "Response Header for".
        // Whichever is closer (higher index) = the correct scope.
        String sourceType = extractSourceType(codeGen, dynIdx);

        // SECTION BOUNDARIES:
        // Start = position of the nearest Response Body/Header marker above value
        // End   = position of the next Response Body/Header marker below value
        int sectionStart = findSectionStart(codeGen, dynIdx);
        int sectionEnd   = findSectionEnd(codeGen, dynIdx + dyn.length());

        // Use a tight 500-char window around the value for boundary extraction.
        // The full section can be megabytes of HTML; boundaries are always
        // within a few chars of the value itself.
        int ctxLeft  = Math.max(0, dynIdx - 500);
        int ctxRight = Math.min(codeGen.length(), dynIdx + dyn.length() + 500);
        String tightContext  = codeGen.substring(ctxLeft, ctxRight);
        int dynIdxInCtx = dynIdx - ctxLeft;

        String[] bounds = extractBoundaries(dyn, tightContext, dynIdxInCtx);

        // For ordinal matching, use the full section
        String snapshotContext = codeGen.substring(
            Math.max(0, sectionStart),
            Math.min(codeGen.length(), sectionEnd));
        if (snapshotContext.indexOf(dyn) < 0) {
        ...
        int ordinal      = findOrdinal(all, dyn);
        String paramName = generateParamName(snapshotId);
    ...
    // PARAM NAME
    // =========================================================================
    private String generateParamName(String snapshotId) {
        String base = snapshotId.replaceAll("\\.inf$", "")
    ...
    private void showStudio(CorrelationResult r) {
        ...
        setStatus("Correlation generated. Click Test to verify boundaries.", STATUS_INF);
    ...
    private void runTest() {
        ...
        // Use rawCodeGenText; fall back to codeGenPane text if not loaded yet
        String testScope = rawCodeGenText;
        if (testScope == null || testScope.isEmpty())
            testScope = codeGenPane.getText();
        if (testScope == null || testScope.isEmpty()) {
            setStatus("[FAIL] CodeGen file not loaded. Please load the CodeGen file first.", STATUS_ERR);
            return;
        }
        ...
        // Check if the dynamic value exists at all in codegen
        if (!testScope.contains(currentResult.dynamicValue)) {
            setStatus("[FAIL] Dynamic value not found in CodeGen. Wrong file loaded?", STATUS_ERR);
            return;
        }
        ...
        boolean passed = currentResult.dynamicValue.equals(extracted);
    ...
    void highlightBoundariesInCodeGen(String lb, String value, String rb, int sectionStart) {
        StyledDocument doc = codeGenPane.getStyledDocument();
        // Use rawCodeGenText for position calculations - it matches what setText() loaded
        // and positions are stable (line endings normalised on load).
        String text = rawCodeGenText;
        if (text == null || text.isEmpty()) text = codeGenPane.getText();

        // Step 1: locate the dynamic value itself in the pane text.
        // This is the anchor - always correct regardless of LB/RB.
        int valueIdx = text.indexOf(value);
        if (valueIdx < 0) return;  // not in pane at all

        // Step 2: try to find LB immediately before the value.
        // Walk backward from valueIdx to see if LB is right there.
        int lbIdx   = -1;
        int valStart = valueIdx;
        int valEnd   = valueIdx + value.length();

        if (!lb.isEmpty()) {
            // Check if lb sits right before the value (exact match)
            int expectedLbPos = valueIdx - lb.length();
            if (expectedLbPos >= 0 && text.substring(expectedLbPos, valueIdx).equals(lb)) {
                lbIdx    = expectedLbPos;
                valStart = valueIdx;
            } else {
                // Search forward from the last newline before valueIdx
                // to find lb+value on the same logical line / JSON block
        ...
        SimpleAttributeSet dynA = new SimpleAttributeSet();
        StyleConstants.setBackground(dynA, HL_DYNAMIC_BG);
        StyleConstants.setForeground(dynA, HL_DYNAMIC_FG);
        StyleConstants.setBold(dynA, true);

        SimpleAttributeSet bndA = new SimpleAttributeSet();
        StyleConstants.setBackground(bndA, HL_BOUNDARY_BG);
        StyleConstants.setForeground(bndA, HL_BOUNDARY_FG);

        // Highlight LB (green) if found
        if (lbIdx >= 0) {
            doc.setCharacterAttributes(lbIdx, lb.length(), bndA, false);
        }
        // Highlight dynamic value (orange) - always
        doc.setCharacterAttributes(valStart, value.length(), dynA, false);
        // Highlight RB (green) if present right after value
        if (!rb.isEmpty() && valEnd < text.length() &&
            text.startsWith(rb, valEnd)) {
            doc.setCharacterAttributes(valEnd, rb.length(), bndA, false);
        }
    ...
    // FILE LOADING
    // =========================================================================
    private void loadFile(JTextPane tp, boolean isAction) {
    ...
    static class LineNumberGutter extends JPanel implements DocumentListener {
        ...
        protected void paintComponent(Graphics g) {
            ...
            try {
                int lh = tp.getFontMetrics(tp.getFont()).getHeight();
                Rectangle clip = g.getClipBounds();
                int start = clip.y / lh;
                int end   = (clip.y + clip.height) / lh + 1;
                int total = tp.getDocument().getDefaultRootElement().getElementCount();
                for (int i = start; i <= Math.min(end, total - 1); i++) {
                    String num = String.valueOf(i + 1);
                    int y = (i * lh) + lh - 3;
                    g.drawString(num, getWidth() - g.getFontMetrics().stringWidth(num) - 4, y);
                }
            } catch (Exception ignored) {}
        }
    ...
    class SearchPanel extends JPanel {
        ...
        private void doSearch() {
            ...
            if (query == null || query.trim().isEmpty()) { countLabel.setText("  "); return; }
        ...
        private void replaceAll() {
            ...
            try {
                int flags = caseBox.isSelected() ? 0 : Pattern.CASE_INSENSITIVE;
                String pat = regexBox.isSelected() ? query : Pattern.quote(query);
                String result = Pattern.compile(pat, flags).matcher(target.getText())
        ...
        private void highlight() {
            ...
            StyleConstants.setBackground(base, BG_EDITOR);
            StyleConstants.setForeground(base, FG_DEFAULT);
            doc.setCharacterAttributes(0, text.length(), base, false);
            if (target == actionPane) applySyntax(target);
            for (int i = 0; i < matches.size(); i++) {
                int[] m = matches.get(i);
                SimpleAttributeSet a = new SimpleAttributeSet();
                StyleConstants.setBackground(a, i == currentIdx ? HL_CURRENT_BG : HL_SEARCH_BG);
                StyleConstants.setForeground(a,
                    i == currentIdx ? new Color(0xff, 0xe4, 0x4d) : new Color(0xff, 0xff, 0x80));
                doc.setCharacterAttributes(m[0], m[1] - m[0], a, false);
            }
        }
        @SuppressWarnings("deprecation")
        private void scrollTo(int idx) {
            ...
            try { target.scrollRectToVisible(target.modelToView(pos)); }
            catch (BadLocationException ignored) {}
        }
