...
public class LRCorrelator extends JFrame {
    ...
    private void attachActionContextMenu(JTextPane tp) {
        ...
        menu.addSeparator();   menu.add(findInCg); menu.add(copyItem);
    ...
    private void insertErrorHandling() {
        ...
        String endCur = (curTxn != null)
            ? I + "    lr_end_transaction(\"" + escapeC(curTxn) + "\", LR_FAIL);\n" : "";
        ...
        String check =
            ...
            I + "    lr_error_message(\"[EH] Text not found: '" + escapeC(text) +
                "'. Jumping to " + escapeC(txn) + ".\");\n" +
            endCur +
            I + "    goto " + label + ";\n" +
            I + "}";
    ...
    private void insertLoop() {
        ...
        String countExpr = count.matches("\\d+") ? count
        ...
        String wrapper =
            "    // loop x" + count + ": transactions \"" + start + "\" .. \"" + end + "\"\n" +
            "    {\n" +
            "        int " + var + ";\n" +
            "        for (" + var + " = 0; " + var + " < " + countExpr + "; " + var + "++) {\n" +
            indented + "\n" +
            "        }\n" +
            "    }";
        String result = action.substring(0, bodyStart) + wrapper + action.substring(bodyEnd);
        actionPane.setText(result);
        rawActionText = result;
        applySyntax(actionPane);
        try { actionPane.setCaretPosition(Math.min(bodyStart, result.length())); } catch (Exception ignored) {}
        setStatus("[LOOP] Wrapped \"" + start + "\" .. \"" + end + "\" in a loop x" + count + ".", STATUS_OK);
    }
