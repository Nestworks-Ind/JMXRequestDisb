...
public class LRCorrelator extends JFrame {
    ...
    private JPanel buildToolbar() {
        ...
        JButton automate = toolButton("Automate Params", BTN_GREEN);
        automate.addActionListener(e -> automateParamsDialog());
        automate.setToolTipText("Auto-correlate chosen parameters (e.g. pzuiactionzzz, pzTransactionId) across the whole Action file");
        JButton requests = toolButton("Requests", C_PARAM);
        requests.addActionListener(e -> requestsDialog());
        requests.setToolTipText("Group requests by host, search, and disable / enable / delete them in bulk");
        ...
        bar.add(loadAction); bar.add(loadCodeGen); bar.add(newSession); bar.add(standardize); bar.add(dynReq); bar.add(automate); bar.add(requests);
    ...
    // =========================================================================
    // AUTOMATE PARAMETER CORRELATION (e.g. pzuiactionzzz, pzTransactionId)
    // =========================================================================
    private void automateParamsDialog() {
        refreshRawFromPanes();
        if (rawActionText == null || rawActionText.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Load or paste the Action file first.", "Automate", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (rawCodeGenText == null || rawCodeGenText.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Load or paste the Code Generation file first.", "Automate", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        JCheckBox cbAction = darkCheck("pzuiactionzzz", true);
        JCheckBox cbTxn    = darkCheck("pzTransactionId", true);
        cbAction.setAlignmentX(Component.LEFT_ALIGNMENT);
        cbTxn.setAlignmentX(Component.LEFT_ALIGNMENT);
        JLabel more = darkLabel("Other parameters (comma-separated):");
        more.setAlignmentX(Component.LEFT_ALIGNMENT);
        JTextField tfMore = new JTextField();
        styleSmallField(tfMore);
        tfMore.setMaximumSize(new Dimension(Integer.MAX_VALUE, 26));
        tfMore.setAlignmentX(Component.LEFT_ALIGNMENT);
        body.add(cbAction);
        ...
        int ok = JOptionPane.showConfirmDialog(this, body, "Automate Parameter Correlation",
            JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        if (ok != JOptionPane.OK_OPTION) return;

        java.util.LinkedHashSet<String> params = new java.util.LinkedHashSet<>();
        if (cbAction.isSelected()) params.add("pzuiactionzzz");
        if (cbTxn.isSelected())    params.add("pzTransactionId");
        for (String s : tfMore.getText().split(",")) { s = s.trim(); if (!s.isEmpty()) params.add(s); }
        if (params.isEmpty()) { JOptionPane.showMessageDialog(this, "Select at least one parameter.", "Automate", JOptionPane.WARNING_MESSAGE); return; }
        automateCorrelate(new ArrayList<>(params));
    }

    private void automateCorrelate(List<String> params) {
        String original = rawActionText;
        String codeGen  = rawCodeGenText;
        String action   = original;                 // accumulates edits
        int done = 0, skipped = 0;
        java.util.LinkedHashSet<String> seen = new java.util.LinkedHashSet<>();
        StringBuilder log = new StringBuilder();

        for (String param : params) {
            for (String dyn : findParamValues(original, param)) {
                String clean = cleanActionValue(dyn);
                if (clean.isEmpty() || clean.startsWith("{")) continue;   // already parameterised
                if (!seen.add(clean)) continue;                          // this value already handled

                CorrelationResult r = runEngine(dyn, codeGen);
                if (r == null) { skipped++; log.append("  - ").append(param).append(": value not found in CodeGen\n"); continue; }

                // direction guard: source response must be earlier than first use
                String usageSnap = findUsageSnapshot(original, dyn);
                int srcN = snapshotNumber(r.snapshotId), useN = snapshotNumber(usageSnap);
                if (srcN >= 0 && useN >= 0 && srcN >= useN) { skipped++; log.append("  - ").append(param).append(": no earlier source (").append(r.snapshotId).append(")\n"); continue; }

                action = insertBeforeSnapshot(action, r.snapshotId, r.generatedCode);
                action = seamAwareReplace(action, clean, "{" + r.paramName + "}");
                done++;
                log.append("  + ").append(r.paramName).append("  (").append(param).append(", src ").append(r.snapshotId).append(")\n");
            }
        }

        actionPane.setText(action);
        rawActionText = action;
        applySyntax(actionPane);
        actionPane.setCaretPosition(0);
        setStatus("[AUTO] Correlated " + done + " value(s); " + skipped + " skipped.", STATUS_OK);
        JOptionPane.showMessageDialog(this,
            "Automated correlation complete.\n\nCorrelated : " + done + "\nSkipped    : " + skipped +
            "\n\n" + (log.length() == 0 ? "(nothing found)" : log.toString()),
            "Automate", JOptionPane.INFORMATION_MESSAGE);
    }

    /** Find every dynamic value carried by <param> in the Action file, in both the
     *  query form  param=VALUE  and the ITEMDATA form  "Name=param","Value=VALUE". */
    private List<String> findParamValues(String action, String param) {
        List<String> out = new ArrayList<>();
        Matcher a = Pattern.compile(Pattern.quote(param) + "=").matcher(action);
        while (a.find()) { String v = extractParamValue(action, a.end(), true);  if (!v.isEmpty()) out.add(v); }
        Matcher b = Pattern.compile("Name=" + Pattern.quote(param) + "\"\\s*,\\s*\"Value=").matcher(action);
        while (b.find()) { String v = extractParamValue(action, b.end(), false); if (!v.isEmpty()) out.add(v); }
        return out;
    }

    /** Read a value forward from `start`, collapsing/keeping VuGen "..." "..." wrap
     *  seams so long wrapped values are captured whole. Stops at '&' (query) or a
     *  non-seam closing quote (end of the literal). */
    private String extractParamValue(String a, int start, boolean stopAtAmp) {
        int n = a.length(), i = start;
        // leading seam: value wrapped onto the next literal ( ...param=" \n\t "VALUE )
        if (i < n && a.charAt(i) == '"') {
            int j = i + 1; while (j < n && Character.isWhitespace(a.charAt(j))) j++;
            if (j < n && a.charAt(j) == '"') i = j + 1; else return "";
        }
        StringBuilder sb = new StringBuilder();
        while (i < n) {
            char c = a.charAt(i);
            if (stopAtAmp && c == '&') break;
            if (c == '"') {
                int j = i + 1; while (j < n && Character.isWhitespace(a.charAt(j))) j++;
                if (j < n && a.charAt(j) == '"') {                 // seam
                    if (stopAtAmp && j + 1 < n && a.charAt(j + 1) == '&') break;  // seam then '&' -> value ended
                    sb.append(a, i, j + 1); i = j + 1; continue;   // internal seam -> keep and continue
                }
    ...
    // =========================================================================
    // REQUESTS MANAGER (group by host, search, disable / enable / delete)
    // =========================================================================
    private static class ReqBlock {
        int start, end; String text, host, path, name, method, category; boolean disabled;
    }

    private List<ReqBlock> parseRequestBlocks(String action) {
        List<ReqBlock> out = new ArrayList<>();
        if (action == null) return out;
        Matcher fn = Pattern.compile(
            "(?im)^([ \\t]*)(//)?[ \\t]*web_(?:url|submit_data|custom_request|submit_form|link)\\s*\\(").matcher(action);
        Pattern lastP = Pattern.compile("(?im)\\bLAST\\s*\\)\\s*;");
        // a contiguous run of web_add_* (header/cookie) statements right before a request.
        // Match each statement up to its closing ")" + ";" (DOTALL) so a ';' inside a
        // cookie value doesn't cut it short.
        Pattern hdrRun = Pattern.compile(
            "(?is)((?:\\s*(?://)?[ \\t]*web_add_(?:auto_header|header|cookie)\\s*\\(.*?\\)\\s*;)+)\\s*$");
        int from = 0, prevEnd = 0;
        while (fn.find(from)) {
            int reqStart = fn.start();
            boolean disabled = fn.group(2) != null;
            Matcher lm = lastP.matcher(action);
            int end = lm.find(fn.end()) ? lm.end() : action.length();

            // pull in the associated headers/cookies that immediately precede this request
            int blockStart = reqStart;
            Matcher hm = hdrRun.matcher(action.substring(prevEnd, reqStart));
            if (hm.find()) blockStart = prevEnd + hm.start(1);

            String block = action.substring(blockStart, end);
            ReqBlock b = new ReqBlock();
            b.start = blockStart; b.end = end; b.text = block; b.disabled = disabled;
            b.name   = firstGroup(block, "(?is)web_(?:url|submit_data|custom_request|submit_form|link)\\s*\\(\\s*\"([^\"]*)\"");
            b.host   = firstGroup(block, "(?i)\"URL=https?://([^/\"]+)");
            b.path   = firstGroup(block, "(?i)\"URL=https?://[^/\"]+([^\"?]*)");
            b.method = firstGroup(block, "(?i)\"Method=([A-Za-z]+)\"");
            if (b.host == null || b.host.isEmpty()) b.host = "(relative / no URL)";
            if (b.method == null || b.method.isEmpty()) b.method = "";
            b.category = categorize(b);
            out.add(b);
            from = end; prevEnd = end;
        }
        return out;
    }
    // classify a request by the static extension in its URL/path/name
    private String categorize(ReqBlock b) {
        String s = ((b.path == null ? "" : b.path) + " " + (b.name == null ? "" : b.name)).toLowerCase();
        Matcher m = Pattern.compile("\\.([a-z0-9]{2,5})(?=$|[?\"&' ])").matcher(s);
        String ext = ""; while (m.find()) ext = m.group(1);
        switch (ext) {
            case "js": case "mjs": return "JS";
            case "css": return "CSS";
            case "png": case "jpg": case "jpeg": case "gif": case "svg": case "ico": case "bmp": case "webp": return "Images";
            case "woff": case "woff2": case "ttf": case "eot": case "otf": return "Fonts";
            case "json": case "xml": case "txt": case "map": return "Other static";
            default: return "Dynamic";
        }
    }
    private String firstGroup(String s, String regex) {
        Matcher m = Pattern.compile(regex).matcher(s);
        return m.find() ? m.group(1) : "";
    }

    private void requestsDialog() {
        refreshRawFromPanes();
        if (rawActionText == null || rawActionText.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Load or paste the Action file first.", "Requests", JOptionPane.WARNING_MESSAGE);
            return;
        }
        final List<ReqBlock>[] blocksRef = new List[]{ parseRequestBlocks(rawActionText) };

        JDialog d = new JDialog(this, "Requests - disable / enable / delete", true);
        d.getContentPane().setBackground(BG_PANEL);
        d.setLayout(new BorderLayout());

        // ---- By Host tab ----
        DefaultTableModel hostModel = new DefaultTableModel(new Object[]{"", "Host", "Requests", "Enabled", "Disabled"}, 0){
        ...
        styleReqTable(hostTable); hostTable.getColumnModel().getColumn(0).setMaxWidth(28);

        // ---- By Type tab (static content grouping) ----
        DefaultTableModel typeModel = new DefaultTableModel(new Object[]{"", "Type", "Requests", "Enabled", "Disabled"}, 0){
            public Class<?> getColumnClass(int c){ return c==0 ? Boolean.class : Object.class; }
            public boolean isCellEditable(int r, int c){ return c==0; }
        };
        JTable typeTable = new JTable(typeModel);
        styleReqTable(typeTable); typeTable.getColumnModel().getColumn(0).setMaxWidth(28);

        // ---- Search tab ----
        JTextField tfSearch = new JTextField(); styleSmallField(tfSearch);
        JCheckBox cbUrl = darkCheck("URL/Host", true), cbPath = darkCheck("Path", true),
                  cbName = darkCheck("Name", true), cbWhole = darkCheck("Whole request (URL/body/headers)", false),
                  cbRegex = darkCheck("Regex", false), cbCase = darkCheck("Case", false);
        DefaultTableModel searchModel = new DefaultTableModel(new Object[]{"", "Name", "Method", "Host", "Path", "State"}, 0){
            public Class<?> getColumnClass(int c){ return c==0 ? Boolean.class : Object.class; }
            public boolean isCellEditable(int r, int c){ return c==0; }
        };
        JTable searchTable = new JTable(searchModel);
        styleReqTable(searchTable); searchTable.getColumnModel().getColumn(0).setMaxWidth(28);

        Runnable refreshHosts = () -> {
            hostModel.setRowCount(0);
            java.util.LinkedHashMap<String,int[]> g = new java.util.LinkedHashMap<>();
            for (ReqBlock b : blocksRef[0]) {
                int[] c = g.computeIfAbsent(b.host, k -> new int[3]);
                c[0]++; if (b.disabled) c[2]++; else c[1]++;
            }
            for (Map.Entry<String,int[]> e : g.entrySet())
                hostModel.addRow(new Object[]{ Boolean.FALSE, e.getKey(), e.getValue()[0], e.getValue()[1], e.getValue()[2] });
        };
        Runnable refreshTypes = () -> {
            typeModel.setRowCount(0);
            String[] order = {"JS","CSS","Images","Fonts","Other static","Dynamic"};
            java.util.LinkedHashMap<String,int[]> g = new java.util.LinkedHashMap<>();
            for (String k : order) g.put(k, new int[3]);
            for (ReqBlock b : blocksRef[0]) {
                int[] c = g.get(b.category); if (c==null) continue;
                c[0]++; if (b.disabled) c[2]++; else c[1]++;
            }
            for (String k : order) { int[] c = g.get(k); if (c[0]>0) typeModel.addRow(new Object[]{ Boolean.FALSE, k, c[0], c[1], c[2] }); }
        };
        Runnable runSearch = () -> {
            searchModel.setRowCount(0);
            String term = tfSearch.getText();
            Pattern pat = null;
            if (cbRegex.isSelected() && !term.isEmpty())
                try { pat = Pattern.compile(term, cbCase.isSelected()?0:Pattern.CASE_INSENSITIVE); } catch (Exception ex) { pat=null; }
            for (ReqBlock b : blocksRef[0]) {
                boolean match = term.isEmpty();
                if (!match) {
                    String h;
                    if (cbWhole.isSelected()) h = b.text;        // match anywhere in the request (URL/body/headers)
                    else {
                        StringBuilder hay = new StringBuilder();
                        if (cbUrl.isSelected())  hay.append(b.host).append(' ');
                        if (cbPath.isSelected()) hay.append(b.path).append(' ');
                        if (cbName.isSelected()) hay.append(b.name).append(' ');
                        h = hay.toString();
                    }
                    if (pat != null) match = pat.matcher(h).find();
                    else match = cbCase.isSelected() ? h.contains(term) : h.toLowerCase().contains(term.toLowerCase());
                }
                if (match) searchModel.addRow(new Object[]{ Boolean.FALSE, b.name, b.method, b.host, b.path, b.disabled?"Disabled":"Enabled" });
            }
        };
        refreshHosts.run(); refreshTypes.run(); runSearch.run();

        JButton searchBtn = toolButton("Search", BTN_BLUE); searchBtn.addActionListener(e -> runSearch.run());
        tfSearch.addActionListener(e -> runSearch.run());
        JPanel sBar = new JPanel(new FlowLayout(FlowLayout.LEFT, 4, 4)); sBar.setBackground(BG_META);
        sBar.add(darkLabel("Find:")); tfSearch.setColumns(18); sBar.add(tfSearch);
        sBar.add(cbUrl); sBar.add(cbPath); sBar.add(cbName); sBar.add(cbWhole); sBar.add(cbRegex); sBar.add(cbCase); sBar.add(searchBtn);
        JPanel searchPanel = new JPanel(new BorderLayout()); searchPanel.setBackground(BG_PANEL);
        searchPanel.add(sBar, BorderLayout.NORTH); searchPanel.add(new JScrollPane(searchTable), BorderLayout.CENTER);

        JTabbedPane tabs = new JTabbedPane();
        tabs.addTab("By Host", new JScrollPane(hostTable));
        tabs.addTab("By Type", new JScrollPane(typeTable));
        tabs.addTab("Search", searchPanel);

        // ---- action buttons ----
        JButton disableB = toolButton("Disable", BTN_YELLOW);
        JButton enableB  = toolButton("Enable",  BTN_GREEN);
        JButton deleteB  = toolButton("Delete",  new Color(0xe0,0x6c,0x6c));
        JButton closeB   = toolButton("Close",   FG_MUTED);
        java.util.function.IntConsumer act = mode -> {   // 0 disable, 1 enable, 2 delete
            List<ReqBlock> targets = collectTargets(tabs.getSelectedIndex(), hostTable, hostModel, typeTable, typeModel, searchTable, searchModel, blocksRef[0]);
            if (targets.isEmpty()) { JOptionPane.showMessageDialog(d, "Select host(s), type(s) or request(s) first (tick the checkboxes).", "Requests", JOptionPane.INFORMATION_MESSAGE); return; }
            if (mode == 2 && JOptionPane.showConfirmDialog(d, "Delete " + targets.size() + " request(s) and their headers/cookies? This cannot be undone here.", "Delete", JOptionPane.OK_CANCEL_OPTION) != JOptionPane.OK_OPTION) return;
            String action = applyReqAction(rawActionText, targets, mode);
            actionPane.setText(action); rawActionText = action; applySyntax(actionPane);
            blocksRef[0] = parseRequestBlocks(action);
            refreshHosts.run(); refreshTypes.run(); runSearch.run();
            setStatus("[REQ] " + (mode==0?"Disabled ":mode==1?"Enabled ":"Deleted ") + targets.size() + " request(s) + headers/cookies.", STATUS_OK);
        };
        disableB.addActionListener(e -> act.accept(0));
        enableB.addActionListener(e -> act.accept(1));
        deleteB.addActionListener(e -> act.accept(2));
        closeB.addActionListener(e -> d.dispose());
        JPanel btns = new JPanel(new FlowLayout(FlowLayout.RIGHT, 8, 8)); btns.setBackground(BG_META);
        btns.add(disableB); btns.add(enableB); btns.add(deleteB); btns.add(closeB);
    ...
    private void styleReqTable(JTable t) {
        t.setBackground(BG_CORR); t.setForeground(FG_DEFAULT);
        t.setGridColor(BORDER); t.setRowHeight(22);
        t.getTableHeader().setBackground(BG_META); t.getTableHeader().setForeground(FG_DEFAULT);
        t.setSelectionBackground(new Color(0x2a,0x2a,0x4a));
    }

    // gather the blocks targeted by the active tab's checkboxes
    private List<ReqBlock> collectTargets(int tab, JTable hostTable, DefaultTableModel hostModel,
                                          JTable typeTable, DefaultTableModel typeModel,
                                          JTable searchTable, DefaultTableModel searchModel, List<ReqBlock> blocks) {
        List<ReqBlock> targets = new ArrayList<>();
        if (tab == 0) {                                   // By Host
            java.util.Set<String> hosts = new java.util.HashSet<>();
            for (int i = 0; i < hostModel.getRowCount(); i++)
                if (Boolean.TRUE.equals(hostModel.getValueAt(i, 0))) hosts.add(String.valueOf(hostModel.getValueAt(i, 1)));
            for (ReqBlock b : blocks) if (hosts.contains(b.host)) targets.add(b);
        } else if (tab == 1) {                            // By Type (static content)
            java.util.Set<String> types = new java.util.HashSet<>();
            for (int i = 0; i < typeModel.getRowCount(); i++)
                if (Boolean.TRUE.equals(typeModel.getValueAt(i, 0))) types.add(String.valueOf(typeModel.getValueAt(i, 1)));
            for (ReqBlock b : blocks) if (types.contains(b.category)) targets.add(b);
        } else {                                          // Search (individual requests)
            for (int i = 0; i < searchModel.getRowCount(); i++) {
                if (!Boolean.TRUE.equals(searchModel.getValueAt(i, 0))) continue;
                String nm = str(searchModel.getValueAt(i,1)), ho = str(searchModel.getValueAt(i,3)), pa = str(searchModel.getValueAt(i,4));
                for (ReqBlock b : blocks)
                    if (!targets.contains(b) && nm.equals(b.name) && ho.equals(b.host) && pa.equals(b.path)) { targets.add(b); break; }
            }
        }
        return targets;
    }
    ...
    // apply disable(0) / enable(1) / delete(2) to the target blocks; edit high offset first
    private String applyReqAction(String action, List<ReqBlock> targets, int mode) {
        targets.sort((a,b) -> Integer.compare(b.start, a.start));
        StringBuilder sb = new StringBuilder(action);
        for (ReqBlock b : targets) {
            String repl;
            if (mode == 0)      repl = b.disabled ? b.text : commentBlock(b.text);
            else if (mode == 1) repl = b.disabled ? uncommentBlock(b.text) : b.text;
            else                repl = "";   // delete
            sb.replace(b.start, b.end, repl);
        }
        return sb.toString();
    }
    private String commentBlock(String t) {
        StringBuilder sb = new StringBuilder();
        String[] lines = t.split("\n", -1);
        for (int i=0;i<lines.length;i++){ if(i>0) sb.append("\n");
            String ln=lines[i]; sb.append(ln.trim().isEmpty()||ln.trim().startsWith("//") ? ln : "//"+ln); }
        return sb.toString();
    }
    private String uncommentBlock(String t) {
        StringBuilder sb = new StringBuilder();
        String[] lines = t.split("\n", -1);
        for (int i=0;i<lines.length;i++){ if(i>0) sb.append("\n"); sb.append(lines[i].replaceFirst("^([ \\t]*)//","$1")); }
        return sb.toString();
    }
    ...
    private void doCorrelate() {
        ...
        modified = seamAwareReplace(modified,
            cleanActionValue(currentResult.dynamicValue), "{" + currentResult.paramName + "}");
    ...
    /** Replace a value that may be split across VuGen "..." "..." wrap seams.
     *  Flattens the seams, finds the logical (clean) value, and replaces the exact
     *  wrapped span in the raw text with {param} - keeping the literal structure. */
    private String seamAwareReplace(String action, String cleanValue, String replacement) {
        if (cleanValue == null || cleanValue.isEmpty()) return action;
        int n = action.length();
        StringBuilder flat = new StringBuilder(); int[] map = new int[n + 1]; int fi = 0, i = 0;
        while (i < n) {
            if (action.charAt(i) == '"') {                     // possible seam: " \s* "
                int j = i + 1; while (j < n && Character.isWhitespace(action.charAt(j))) j++;
                if (j < n && action.charAt(j) == '"') { i = j + 1; continue; }  // skip the seam
            }
            map[fi] = i; flat.append(action.charAt(i)); fi++; i++;
        }
        map[fi] = n;
        String f = flat.toString();
        StringBuilder out = new StringBuilder(); int last = 0, from = 0, idx;
        while ((idx = f.indexOf(cleanValue, from)) >= 0) {
            int rawStart = map[idx];
            int rawEnd   = map[idx + cleanValue.length() - 1] + 1;
            out.append(action, last, rawStart).append(replacement);
            last = rawEnd; from = idx + cleanValue.length();
        }
        out.append(action.substring(last));
        return out.toString();
    }
