...
public class LRCorrelator extends JFrame {
    ...
    private JPanel buildToolbar() {
        ...
        bar.add(loadAction); bar.add(loadCodeGen); bar.add(newSession); bar.add(standardize);
    ...
    // Comment out (line-comment, so embedded */ can't break it) each
    // web_url/web_custom_request block whose URL is a static resource. count[0]
    // receives the number disabled.
    private String stdDisableStatic(String action, int[] count) {
        ...
        while (fn.find(from)) {
            ...
            if (end < 0) break;
            if (staticUrl.matcher(action.substring(start, end)).find())
                spans.add(new int[]{start, end});
            from = end;
        }
        if (spans.isEmpty()) return action;
        StringBuilder sb = new StringBuilder();
        int last = 0;
        for (int[] s : spans) {
            sb.append(action, last, s[0]);
            sb.append("// [NestWorks] static resource disabled\n");
            sb.append("//").append(action.substring(s[0], s[1]).replace("\n", "\n//"));
            last = s[1];
            count[0]++;
        }
        sb.append(action.substring(last));
